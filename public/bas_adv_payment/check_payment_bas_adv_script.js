window.addEventListener('DOMContentLoaded', async () => {
    ...
    tg.MainButton.onClick(() => {
        ...
        fetch('/check-user-bas-adv-payment', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ userEmailOrPhone: userEmailOrPhone }),
        })
            ...
            .then(data => {
                ...
                if (data.data === 'error') {
                    ...
                    title.style.backgroundImage = 'linear-gradient(0deg, #FF00C7 0.02%, #FF003D 99.35%)'
                    title.textContent = 'Извините, произошла ошибка. Попробуйте ещё раз!' 
                    input.value = ''
                    tg.MainButton.hide() 
                } else {
                    const dataToSend = JSON.stringify({
                        webAppType: 'basic-advanced',
